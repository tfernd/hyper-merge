...
def extract_lora(
    ...
    *,
    dim: int | float = 0.08,
    conv_dim: Optional[int | float] = None, # TODO add support
    min_dim: int = 1,
    max_dim: int = 1_024,
    ...
    conv_dim = dim if conv_dim is None else conv_dim
    ...
    for key, lora_key in tqdm(SD15MAP.items(), "Converting to LORA"):
        ...
        N = diff.size(1)
        kernel_size = shape[2:]
        rank = clamp_rank(dim, N, min_dim, min(max_dim, N))
        conv_rank = clamp_rank(conv_dim, N, min_dim, min(max_dim, N))   

        is_conv = len(kernel_size) == 2 and kernel_size == (3, 3)
        general_rank = rank if not is_conv else conv_rank

        U, Vh = svd(diff, general_rank, shape)
        alpha = torch.tensor(general_rank).half()

        out[lora_key + ".lora_up.weight"] = U.cpu()
        out[lora_key + ".lora_down.weight"] = Vh.cpu()
        out[lora_key + ".alpha"] = alpha

    if save_path is not None:
        # Not a file # TODO is there a better way?
